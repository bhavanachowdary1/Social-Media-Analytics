    ...
    for index,row in data.iterrows():
        if len(colName)==0 and len(dataToCount)==0 or row[colName]==dataToCount:
            ...
            if state not in dictionary:
                dictionary[state]=0
            dictionary[state]=dictionary[state]+1
    return dictionary
...
def getHashtagRates(data):
    ...
    for index,row in data.iterrows():
        ...
        for j in range(len(hashtag)):
            tag=hashtag[j]
            if tag not in hashtag_dictionary:
                hashtag_dictionary[tag] =0
            hashtag_dictionary[tag] += 1
...
def graphHashtagSentimentByFrequency(data):
    ...
    for i in mostcommon:
        ...
        frequentlist.append(mostcommon[i])
        sentimentlist.append(getHashtagSentiment(data,i))
    scatterPlot(frequentlist,sentimentlist,hashtaglist,"Hashtags Frequency")
    return
